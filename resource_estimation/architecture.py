...
SUPERCOND_GATES = {
    ...
    # 40ns; both values come from Fowler et al. (Barends 2014, page 5).
    cirq.CZ: 0.040,
    # Based on 1us cycle time assumed by Gidney RSA 2025.
    cirq.ResetChannel: 1,
...
def _merge_cost(
    ...
    d is the code distance, k is the number of patches being merged, and
    smooth indicates if patches are being joined at the Z or X boundary.
    """
    assert k >= 2
...
class Architecture(abc.ABC):
    ...
    Class for representing device architectures.
        ...
        Comes preloaded with many primitive costs shared by the current set of
        subclasses.
    """
    ...
    # Since there are many ways to generate T states, each Architecture subclass
    # must specify its particular method.
    @cached_property
    ...
    def _cultivate_y_cost(self):
        """
        Cost estimate for measuring a surface code patch in the Y basis.
        Measuring in the Y basis facilitates gate teleportation the same way
        that cultivating T does.
        The procedure is based on [Inplace access to the Surface Code Y
        Basis](https://arxiv.org/pdf/2302.07395v2).
        The cost estimates were generated by looking carefully at circuits from
        ...
        # TODO: Perhaps canonical cost includes SE before and after, for a
        # total of two more units of SE.
        moment_cost = se_moment_cost + Y_moment_cost + Y_moment_cost
        op_time = self.total_time(moment_cost_dict=moment_cost)

        # For gate cost, approximate with one round of syndrome extraction plus
        # an additional d-1 diagonal of CZ gates.
...
class DefaultMovement(Architecture):
    """
    Class representing the set of primitives available with movement for
    transversal operations.
    This default version assumes a single species of neutral-atom qubits using
    ...
    def syndrome_extract_cost(self, op):
        # Build from base syndrome extraction cost plus movement penalties for
        # CZ and measurement moments.
    ...
    def _cnot_cost(self) -> dict:
        ...
        # TODO: Resolve this with the compiler assumption that we should already
        # have conjugated to CZ by the time we do CNOT.
        moment_cost = {
    ...
    def _h_cost(self) -> dict:
        ...
        # Technically, physical repermutation could be carried out digitally
        # because there are no connectivity constraints.
        moment_cost = {
            cirq.PhasedXZGate: 1,
            cirq.QubitPermutationGate: 1,
        }
    ...
    def _s_cost(self) -> dict:
        """
        Resources the folded transversal S gate from
        https://arxiv.org/pdf/2412.01391.
        It resembles one syndrome-extraction round with CNOT gates across the
        main diagonal, plus physical S/Sdg gates.
        """
    ...
    def move_cost(self, op):
        ...
        If qubits are spaced by ~1um, it takes about 2*d us to move to an
        adjacent patch.
        If the L1 distance between logical qubits A and B is C, penalize
        Move(A, B) with time 2*C*d (up to a maximum of 500us).
        This may overweight alleyway movement, but it provides a concrete rule
...
class DualSpeciesMovement(DefaultMovement):
    ...
    def _cultivate_t_cost(self):
        """
        Cached property for cultivation-circuit cost at this code distance and
        movement model.
        Values are multiplied by the repeat factor for this architecture
        instance.
        """
        base_cultivation_cost = cultivate(
            dsurface=self.d, fold=self.fold_cultiv, fault_distance=self.cultivation_fault_distance
...
class MeasureZonesOnly(DefaultMovement):
    ...
    def syndrome_extract_cost(self, op: cirq.Operation) -> dict:
        """
        Uses lattice-surgery syndrome extraction and adds moves associated with
        measurements.
        Since this is a movement architecture, rounds should remain low,
        consistent with the promise of correlated decoding.
        """
...
def convert_globals_to_phasedxz(architecture: Architecture, cost_with_globals: dict) -> dict:
    """
    Convert costs defined by GR and Rz into PhasedXZ by removing GR and
    replacing Rz with PhasedXZ to represent arbitrary single-qubit rotations.
    """
