...
def test_other_passes(random_circ):
    # If this test and test_deterministic_compilation both fail,
    # that one likely causes the issue in this one.
    circuit = random_circ
    lay = Column(circuit)
...
def test_verbosity(random_circ):
    # TODO: Make this slightly more realistic.
    # It's a visualization tool, so this is lower priority.
    circuit = random_circ
    lay = Column(circuit)
    arc = arch.DefaultLattice()
...
def test_bell_lattice_TF(bell_circuit):
    ...
    # Split moments also do not get idling because, implicitly,
    # they can always be absorbed into a previous moment.
    cirq.testing.assert_has_diagram(
        compiled_bell_circuit,
        textwrap.dedent(
            """
...
def test_bell_lattice_TT(bell_circuit):
    ...
    compiled_bell_circuit
    # yes idling, yes post-op correction
    # Post-op correction adds nothing in this circuit,
    # so this circuit is the same as the previous one.
    cirq.testing.assert_has_diagram(
        compiled_bell_circuit,
        textwrap.dedent(
            """
    ...
    cirq.testing.assert_has_diagram(
        ...
        "\n".join(
            [
                ...
                (
                    "(1, 1): ───CULT(0.785)─────────────────────────────────────────MOVE───@───"
                    "#2─────SE(1)───MOVE_MZ───M('')───MOVE_MZ───SE(1)───R───"
                ),
            ]
        ),
    )
...
def test_t_movement_TF(t_circuit):
    ...
    cirq.testing.assert_has_diagram(
        ...
        "\n".join(
            [
                (
                    "(0, 0): ───SE(1)─────────H───────MOVE────@───────#2──────SE(1)───SE(1)"
                    "───SE(1)───────────────────────────────────"
                ),
                " " * 33 + "│       │       │",
                (
                    "(0, 1): ───SE(1)─────────SE(1)───#2──────X───────MOVE────#2──────X"
                    "───────MOVE────S─────────SE(1)─────────────────"
                ),
                " " * 57 + "│       │       │",
                (
                ...
                " " * 57 + "│       │       │",
                (
                    "(1, 1): ───CULT(0.785)───SE(1)───────────────────────────MOVE────@───────"
                    "#2──────MOVE_MZ───M('')───MOVE_MZ───R───"
                ),
                "",
            ]
        ),
...
def test_t_movement_TT(t_circuit):
    ...
    # yes idling, yes post-op correction
    compiled_t_circuit = cirq.align_left(compiled_t_circuit)
    # This test was updated by aligning left and by reflecting
    # the change that moves cultivation later in the circuit.
    ...
    cirq.testing.assert_has_diagram(
        ...
        "\n".join(
            [
                ...
                " " * 41 + "│       │       │",
                (
                    "(0, 1): ───SE(1)─────────SE(1)───SE(1)───#2──────X───────MOVE────SE(1)────#2"
                    "─────────────X─────────────MOVE────SE(1)───S─────────SE(1)───SE(1)─────────────────"
                ),
                " " * 74 + "│              │             │",
                (
                    "(1, 0): ───CULT(0.785)───SE(1)───SE(1)───SE(1)───SE(1)───SE(1)───SE(1)────┼"
                    "────SE(1)─────┼────SE(1)────┼───────────────────────────────────────────────────────"
                ),
                " " * 74 + "│              │             │",
                (
                    "(1, 1): ───CULT(0.785)───SE(1)───SE(1)───SE(1)────────────────────────────MOVE"
                    "───────────@─────────────#2──────SE(1)───MOVE_MZ───M('')───MOVE_MZ───SE(1)───R───"
                ),
                " " * 73 + "└──────────┘   └──────────┘",
            ]
...
# TODO: Re-introduce the old lattice TF/TT regression tests once the
# expected diagrams are stabilized after cultivation/alignment changes.
