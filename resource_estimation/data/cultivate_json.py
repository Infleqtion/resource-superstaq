...
from resource_estimation.typing import GATE2STR, CountsDict, StrCounts
...
def cultivate_json(savefile: Path | str | None = None, max_dist: int = 25) -> None:
    ...
    for d in tqdm.tqdm(range(3, max_dist + 1, 2)):
        ...
        gidney_cultiv3_costs = _format_counts_dict(
            count_stim_resources(stim_circuit=gidney_cultiv3)
        )
        gidney_cultiv5_costs = _format_counts_dict(
            count_stim_resources(stim_circuit=gidney_cultiv5)
        )
