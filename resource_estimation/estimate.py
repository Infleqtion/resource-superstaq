...
class ResourceEstimator:
    ...
    def serial_circuit_time(self, circuit: cirq.Circuit) -> float:
        ...
        self.validate_circuit_ops(circuit=circuit)
        return sum(
            self.arc.total_time(self.arc.gate_cost(x)) for x in circuit.all_operations()
        )
    ...
    def critical_path(self, circuit: cirq.Circuit, verbose: int = 0) -> list[cirq.Operation]:
        ...
        """
        warnings.warn(
            "This function can be very expensive.\nIf you just want the physical operations or circuit time, use `critical_path_ops` or `parallel_circuit_time` instead.", stacklevel=2
        )
