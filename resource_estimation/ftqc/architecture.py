...
class Architecture(abc.ABC):
    ...
    @cached_property
    def _cultivate_t_cost(
        self,
    ) -> dict[str, dict[type[cirq.Gate], int] | float]:  # pragma: no cover
...
class DefaultMovement(Architecture):
    ...
    def distil_t_cost(self, op: cirq.Operation) -> dict[str, dict[type[cirq.Gate], int] | float]:
        return self._distil_t_cost

    @cached_property
    def _distil_t_cost(self) -> dict[str, dict[type[cirq.Gate], int] | float]:
