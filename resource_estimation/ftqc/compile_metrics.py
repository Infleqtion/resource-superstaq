...
from .factory_specs import ReactionDepth, factory_type_for_gate
...
class ReactionDepthMetricCollector(FTCompileMetricCollector):
    """Collect per-qubit reaction depths from factory-backed replacements.

    Factory gates are identified by converting `input_op.gate` to an `ftype`.
    If `layout.factory_specs` has no spec for that `ftype`, the collector ignores
    the replacement.
    Concrete reaction-dynamics formulas belong to factory correction policies,
    not to layout classes or resource-generation methods.
    ...
    def on_replacement(
        ...
        """Apply factory correction-policy dynamics for a replacement event.

        The collector uses the gate's `ftype` string to select this layout's
        static factory spec for that factory type. Gates without specs are
        ignored because the spec map is the source of truth for supported
        reaction-depth-aware factories.

        Args:
            input_op: Logical operation being replaced. Its qubits define the
                positional order passed into the factory reaction dynamic.
            replacement_ops: Primitive operations emitted for `input_op`. The
                current factory-spec lookup does not inspect these operations, but
                they remain part of the hook contract for collectors that need
        ...
        Raises:
            ValueError: If the selected reaction dynamic returns a number of qubit
                updates different from the arity of `input_op`.
        """
