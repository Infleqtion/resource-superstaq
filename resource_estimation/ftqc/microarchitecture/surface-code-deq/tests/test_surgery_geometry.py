...
def test_retained_and_new_checks_reconstruct_the_merged_code() -> None:
    for basis in ("XX", "ZZ"):
        ...
        expected = RotatedSurfaceCode(layout.width, layout.height).stabilizers(layout.qubit_at)
        actual = [
...
def test_mxx_layout_supports_different_widths_with_a_shared_height() -> None:
    layout = merge_layout(RotatedSurfaceCode(3, 5), RotatedSurfaceCode(5, 5), "XX")
...
def test_mzz_layout_supports_different_heights_with_a_shared_width() -> None:
    layout = merge_layout(RotatedSurfaceCode(5, 3), RotatedSurfaceCode(5, 5), "ZZ")
