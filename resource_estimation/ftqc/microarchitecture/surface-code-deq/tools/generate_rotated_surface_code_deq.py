...
def render_surface_code_library(
    ...
    *,
    ...
    prepare_y_gadget = prepare_y_gadget_text(distance, boundary_rounds=boundary_rounds)
...
def _render_surgery_sections(
    ...
    readout_z = " ".join(f"rec[-{patch_size - index}]" for index in range(distance))
...
# Split by measuring the seam, then one extraction round on
...
def render_mxx(distance: int = 3, *, merged_rounds: int | None = None) -> str:
    ...
    return _render_surgery_sections(
        distance, "XX", merged_rounds=merged_rounds
    ).render()
...
def render_mzz(distance: int = 3, *, merged_rounds: int | None = None) -> str:
    ...
    return _render_surgery_sections(
        distance, "ZZ", merged_rounds=merged_rounds
    ).render()
