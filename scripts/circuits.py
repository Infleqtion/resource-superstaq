...
def map_orbitals(n_imp: int, n_b: int, method: Literal["normal", "impurity_centered", "paired"] ="impurity_centered") -> tuple[list[str], list[str], list[str], list[str], dict[str, int]]:
...
def three_orbital_kanamori_hamiltonian(epsilon_imp: int, epsilon_bath:int, v: npt.NDArray[np.float64], u:int, j_ex:int, n_bath: int) -> openfermion.ops.FermionOperator:
    n_imp = 3
