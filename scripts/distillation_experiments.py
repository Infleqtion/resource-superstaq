    ...
    for d in range(7, 20, 2):
        for arc in available_arcs:
            ...
            primitive_circuit = cirq.synchronize_terminal_measurements(
                cirq.align_left(res.compile_ftqc.ft_compile(layout=layout, arc=arc))
            )[1:]
            estimator = res.estimate.ResourceEstimator(arc)
            _time, _moments, _gates = (
                estimator.parallel_circuit_time(primitive_circuit),
                estimator.parallel_circuit_cost(primitive_circuit),
                estimator.serial_circuit_cost(primitive_circuit),
            )
            data = {
                "op_time": _time,
